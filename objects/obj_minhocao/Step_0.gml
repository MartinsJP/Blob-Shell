...
VAI_SE_FUDE_LUCAS = place_meeting(x,y,segue);
chao = place_meeting(x,y+1,obj_plat);
velh = lengthdir_x(7,dir);
velv = lengthdir_y(7,dir);
dir = point_direction(x,y,segue.x,segue.y);

if(VAI_SE_FUDE_LUCAS){
	global.chegou = true
}
if(segue.y <> 480){
	sprite_index = spr_minhocao_cima
}else sprite_index = spr_minhocao_lado
 

 if(velh > 0) image_xscale = 1;
  if(velh < 0) image_xscale = -1;
  
if(velv > 0 ) image_yscale = 1;
if(velv <  0) image_yscale = -1;
